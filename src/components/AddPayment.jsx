...
function AddPayment() {
  ...
  //Funcion para guardar los datos del pago al hacer click 
  // en el boton 
  const onSubmit = (data) => {
    if (paymentType === "card") {
      updatePayment({
  ...
  //Funcion para regresar al paso anterior en el formulario de compra
  //Regresamos a la confirmación del pedido
  const reviewConfirm = () => {
    updateStepOrder(1);
  }//Fin de reviewOrder
  ...
  return (
    <div className='w-full border-t border-gray-500 mt-4'>
        ...
        {paymentType === "card" && (
          <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)}
              ...
              {/* Fecha de expiracion */}
              <div className='text-sm font-medium text-gray-100'>
                <label htmlFor='expirationDate'>Fecha de expiracion (mm/aa)</label>
              </div>
              <div className='relative flex'>
                <input
      ...
      <div className='bg-zinc-800 p-5 rounded-md w-full pb-2 flex justify-between'>
        <Tooltip content="Revisar pedido" placement="left" className="text-gray-700">
          <button className='bg-transparent hover:bg-lime-700
            ...
            type='button'
          >
            Revisar Orden <GrFormPreviousLink className='mr-2 w-5 h-5' size={30} />
          </button>
        </Tooltip>
        <div>
          <Tooltip content={
            paymentType === "pickup" ? "Finalizar pedido"
              : "Agregar direccion"
          }
          >
            <button className='bg-transparent hover:bg-lime-700
