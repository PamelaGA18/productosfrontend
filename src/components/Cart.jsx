import { IoBagCheckOutline, IoCartOutline, IoTrashBinOutline } from "react-icons/io5"
...
function Cart() {
    const {cart, incProduct, decProduct,
        removeProduct, getTotalProducts,
    ...
    const incrementProduct = (product) => {
        ...
        if (existingProduct.toSell == existingProduct.quantity) {
            ...
            return;
        } else { //Si no ha alcanzado el maximo se incrementa la cantidad
                ...
                <div className="flex flex-col px-4 pb-4">
                    {
                        cart.length > 0 ? (
                                ...
                                <tbody className='text-gray-950 text-xs font-semibold py-1 px-1 text-left'>
                                    {
                                        cart.map((product) => (
                                            <tr key={product._id} className="hover:bg-gray-100">
                                                <td>
                                                    <span className="m-1 text-sm font-bold text-red-500"
                                                        onClick={() => decrementProduct(product)}
                                                    >
                                                        -
                                                    </span>
                                                    {product.toSell}
                                                    <span className="m-1 text-sm font-bold text-green-500"
                            ...
                            <div>
                                <p className="text-center text-green-700">El carrito está vacío</p>
                            </div>
                        )
                    }
                </div>
